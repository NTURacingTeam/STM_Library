...
int main(void) {
    ...
    */
    ...
    /* We use PRINTF_USE_USART1 macro to choose which USART we use to redirect
     * printf/scanf*/
    /* uncomment to try on this!*/
    /*
    int num;
    ...
    */
    ...
    /* When we type a char in Putty, interrupt happens and put the char into a
     * queue*/
    ...
    while(1){
            USART2_PutChar(USART2_Get_Buffer());//should print what you had just
    typed delay_ms(500);
    }
    */
    ...
    */
    ...
    while(1){
            //Update_All();
            printf(" ax: %f, ay: %f, az: %f, gx: %f, gy: %f, gz: %f, mx: %f, my:
    %f, mz: %f \r\n", Read_MPU_Data(Acc, X), Read_MPU_Data(Acc, Y),
            Read_MPU_Data(Acc, Z),
            Read_MPU_Data(Gro, X),
            Read_MPU_Data(Gro, Y),
            Read_MPU_Data(Gro, Z),
